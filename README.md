<div align="center">
  <img src="./assets/profile-hero-light.svg#gh-light-mode-only" alt="hrniu" width="100%" />
  <img src="./assets/profile-hero-dark.svg#gh-dark-mode-only" alt="hrniu" width="100%" />
</div>

Java / Spring 后端工程师，专注于可解释智能体系统——RAG、确定性规则、MCP 工具、审计与决策重演。也做浏览器原生小游戏、Codex 工作流，以及一个数学猜想百科。

## 精选项目

<table>
  <tr>
    <td align="center" width="50%">
      <sub>智能体系统</sub><br>
      <a href="https://github.com/hrniux/insurance-underwriting-agent"><img src="./assets/icons/insurance.svg" width="36"></a><br>
      <strong><a href="https://github.com/hrniux/insurance-underwriting-agent">insurance-underwriting-agent</a></strong><br>
      <sub>可解释 Java 智能体：RAG、规则、MCP、审计</sub>
    </td>
    <td align="center" width="50%">
      <sub>后端工程</sub><br>
      <a href="https://github.com/hrniux/spring-boot-lab"><img src="./assets/icons/spring-boot.svg" width="36"></a><br>
      <strong><a href="https://github.com/hrniux/spring-boot-lab">spring-boot-lab</a></strong><br>
      <sub>Java 21 / Spring Boot 实验室</sub>
    </td>
  </tr>
  <tr>
    <td align="center">
      <sub>浏览器产品</sub><br>
      <a href="https://github.com/hrniux/browser-game-arcade"><img src="./assets/icons/game.svg" width="36"></a><br>
      <strong><a href="https://github.com/hrniux/browser-game-arcade">browser-game-arcade</a></strong><br>
      <sub>75 个原生小游戏，在线可玩</sub>
    </td>
    <td align="center">
      <sub>浏览器产品</sub><br>
      <a href="https://github.com/hrniux/harvest-valley-lite"><img src="./assets/icons/harvest.svg" width="36"></a><br>
      <strong><a href="https://github.com/hrniux/harvest-valley-lite">harvest-valley-lite</a></strong><br>
      <sub>轻量 Canvas 农场游戏</sub>
    </td>
  </tr>
  <tr>
    <td align="center">
      <sub>开发工具</sub><br>
      <a href="https://github.com/hrniux/codex-skills"><img src="./assets/icons/codex.svg" width="36"></a><br>
      <strong><a href="https://github.com/hrniux/codex-skills">codex-skills</a></strong><br>
      <sub>Codex / Claude Code 工作流技能</sub>
    </td>
    <td align="center">
      <sub>数学知识</sub><br>
      <a href="https://github.com/hrniux/math-conjectures"><img src="./assets/icons/math.svg" width="36"></a><br>
      <strong><a href="https://github.com/hrniux/math-conjectures">math-conjectures</a></strong><br>
      <sub>数学猜想百科</sub>
    </td>
  </tr>
</table>

## 技术栈

Java 21 · Spring Boot · Spring AI · RAG · MCP · Docker · Maven · JavaScript · Canvas · Vite · Playwright

## 当前在做

可解释智能体系统的评估、重演与治理，以及 Java 后端工程实践的沉淀。
